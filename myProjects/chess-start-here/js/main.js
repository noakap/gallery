...
function checkIsWhite(coord) {
    return (gBoard[coord.i][coord.j] === KING_WHITE ||
        gBoard[coord.i][coord.j] === QUEEN_WHITE ||
        gBoard[coord.i][coord.j] === ROOK_WHITE ||
        gBoard[coord.i][coord.j] === KNIGHT_WHITE ||
        gBoard[coord.i][coord.j] === PAWN_WHITE ||
        gBoard[coord.i][coord.j] === BISHOP_WHITE);
}
...
function getAllPossibleCoordsRook(pieceCoord, isWhite) {
    var res = [];
    for (var i = pieceCoord.j + 1; i < gBoard.length; i++) {
        var nextCoord = { i: pieceCoord.i, j: i };
        res.push(nextCoord);
        if (!isEmptyCell(nextCoord)) {
            if (checkIsWhite(nextCoord) === isWhite) res.pop();
            break;
        }
    }
...
function getAllPossibleCoordsQueen(pieceCoord, isWhite) {
    var coordsBishop = getAllPossibleCoordsBishop(pieceCoord, isWhite);
    var coordsKing = getAllPossibleCoordsKing(pieceCoord, isWhite);
    var res = coordsKing.concat(coordsBishop);
    return res;
}
...
function getAllPossibleCoordsKnight(pieceCoord, isWhite) {
    ...
    for (var i = 0; i < xMove.length; i++) {
        var nextCoord = { i: xMove[i] + pieceCoord.i, j: yMove[i] + pieceCoord.j };
        if (xMove[i] + pieceCoord.i < 0 || xMove[i] + pieceCoord.i > gBoard.length - 1
            || yMove[i] + pieceCoord.j < 0 || yMove[i] + pieceCoord.j > gBoard.length - 1) continue
